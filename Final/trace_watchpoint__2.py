...
def traceit(frame, event, trace_arg):
    ...
    if event == 'line':
        ...
        for var in [var_ for var_ in watchpoints if var_ in frame.f_locals]:
            if var in watch_values and watch_values[var] == frame.f_locals[var]:
                continue

            line, frame_var = frame.f_lineno - 1, frame.f_locals[var]

            if print_line_info:
                print('{0} {1} {2}'.format(event, line, frame.f_code.co_name))
                print_line_info = False

            if var not in watch_values:
                print('{0} : Initialized => {1}'.format(var, repr(frame_var)))

            elif watch_values[var] != frame_var:
                print('{0} : {1} => {2}'.format(var, repr(watch_values[var]), repr(frame_var)))

            watch_values[var] = frame_var

        if not print_line_info:
            debug(input_command(), frame.f_locals)
