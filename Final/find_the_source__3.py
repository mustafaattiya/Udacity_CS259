...
def make_locations(coverage):
    ...
    for line in coverage:
        ...
        result.append((line, counter[line]))
...
def auto_cause_chain(locations):
    ...
    for (line, iteration) in locations:
        ...
        if not state_fail or not state_pass: continue
        ...
        try:
            causes = ddmin(diffs)
            if 'FAIL' == test(causes):
                for var, value in causes:
                    if var not in explored or explored[var] != value:
                        print('Then {0} became {1}'.format(var, repr(value)))
                        explored[var] = value
